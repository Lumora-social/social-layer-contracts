...
fun update_badge_collection_with_events(
    ...
    while (i < len) {
        ...
        // Copy category string since we need to move it into table operations
        let category = std::string::utf8(*std::string::as_bytes(&new_badge.category));

        if (table::contains(&collection.badges, category)) {
            let existing_badge = table::borrow_mut(&mut collection.badges, category);
            ...
            table::add(&mut collection.badges, category, *new_badge);
        };
...
public fun has_badge_category(profile: &Profile, category: String): bool {
    ...
    table::contains(&collection.badges, category)
}
